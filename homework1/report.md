...
### ($n = 100 \sim 10000$)
| 測試數量 ($n$) | 實際高度 ($h$) | 理論高度 ($\log_2 n$) | 比值 (Ratio) |
|:---|:---:|:---:|:---:|
| 100 | 12 | 6.64 | 1.81 |
| 500 | 18 | 8.97 | 2.01 |
| 1000 | 22 | 9.97 | 2.21 |
...
1. **比值 (Ratio) **：
   ...
   - **理論高度** 代表了搜尋的完美平衡。
   - **實際高度** 表現隨機插入下的實際樣子。
   兩者都穩定，隨機BST的平均搜尋路徑長度比平衡樹長，但仍維持在相同的比值。

2. **結構完整性**：
   在執行刪除測試時，透過子節點替換與右子樹最小值替換。
## 申論及開發報告
本實驗讓我深刻理解到「輸入順序」對資料結構效能的巨大影響。雖然隨機輸入能帶來接近 $O(\log n)$ 的表現，但在極端情況下（如遞增資料）仍需依賴 AVL 或紅黑樹等平衡機制來確保效能穩定。
